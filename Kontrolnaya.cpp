...
class weapon {
public:
    weapon(int base_damage, int durability, int degradation, int cost, int weight,int crit_damage); //все важные значения задаются в конструкторе
...
weapon::weapon(int base_damage = 10, int durability = 100, int degradation = 0, int cost = 250, int weight = 5, int crit_damage=0) :
...
int weapon::crit(char&b) {
    int a;
    srand(time(0));
    a = rand() % 12 + 1;
    if (a == 1 && crit_damage>0) {
        crit_check = 100*crit_damage;
        cout << "Произошел Ультра критический урон" << endl;
        degradation += 5;
        cout << "Также произошло повреждение вашего меча на 5 единиц" << endl;
        b = true;
        return crit_check;
    }
    else {
...
int weapon::mini_crit() {
    int a;
    srand(time(0));
    a = rand() % 5 + 1;
    if (a == 1 && crit_damage > 0) {
        crit_check = 10 * crit_damage;
        cout << "Произошел критический урон" << endl;
        degradation += 1;
        cout << "Также произошло повреждение вашего меча на 1 единиц" << endl;
        return crit_check;
    }
    else {
        crit_check = 0;
...
int weapon::hit() {
    ...
    int damage = this->base_damage - random_effect+crit(b); //меняем урон: теперь он варьируется от нуля до изначального базового
    if (b == false) {
        damage += mini_crit();
    }
...
int main() {
   ...
   // weapon simple_iron_sword, players_BIG_sword(20); //создаем два объекта - два меча, себе и противнику
    weapon human_slayer(6, 100, 0, 0, 20, 0);
    weapon  zhumernacht(50, 100, 0, 1000, 5, 2);
    ...
    cout << "Human slayer condition: " << human_slayer.get_durability() - human_slayer.get_degradation() << endl;
    cout << "Zhumernacht condition: " << zhumernacht.get_durability() - zhumernacht.get_degradation() << endl;
